...
export function ProjectCreateWizard({ onCreated }: { onCreated: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="grid gap-10 lg:grid-cols-2 lg:items-start lg:gap-12">
          <div className="space-y-4 lg:pt-2">
            ...
            <p className="text-sm leading-relaxed text-muted-foreground md:text-base">
              Toggle optional Supabase services to match how you&apos;ll use this stack. Your choices are saved with the
              project and shown in the deploy steps. The first run still uses Supabase&apos;s standard Docker bundle
              (services are tightly coupled in upstream compose); finer-grained omission is planned for a later release.
            </p>
          </div>

          <Card className="border-border/80 bg-card/80 shadow-lg shadow-black/20">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        on ? "border-emerald-500/35 bg-emerald-500/[0.06]" : "border-border bg-muted/20",
                      )}
                    >
                  ...
                  {OPTIONAL_SERVICES.map((s) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          "flex gap-3 rounded-xl border p-4 transition-colors",
                          on && !disabled ? "border-emerald-500/35 bg-emerald-500/[0.06]" : "border-border bg-muted/15",
                          disabled && "opacity-60",
                        )}
                      >
      ...
      {step === "details" ? (
        <div className="grid gap-10 lg:grid-cols-2 lg:items-start lg:gap-12">
          <div className="space-y-4 lg:pt-2">
            <h2 className="text-2xl font-semibold tracking-tight text-foreground md:text-3xl">Name &amp; routing</h2>
            <p className="text-sm leading-relaxed text-muted-foreground md:text-base">
              Choose a display name and slug for the project directory, then set the hostnames Traefik should route to
              Kong and optionally Studio.
            </p>
          </div>
      ...
      {step === "deploy" ? (
        <div className="grid gap-10 lg:grid-cols-2 lg:items-start lg:gap-12">
          <div className="space-y-4 lg:pt-2">
            <h2 className="text-2xl font-semibold tracking-tight text-foreground md:text-3xl">Deploy in minutes</h2>
            <p className="text-sm leading-relaxed text-muted-foreground md:text-base">
              Watch your Supabase instance come online. SupaScale downloads the official compose files, injects secrets
              and URLs, writes Traefik labels, and runs{" "}
      ...
      {createWarning && step !== "deploy" ? (
        <div className="rounded-lg border border-amber-500/35 bg-amber-500/5 p-4 text-sm text-amber-200/90">
          {createWarning}
        </div>
      ) : null}
    </div>
