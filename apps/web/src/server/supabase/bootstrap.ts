...
/** Upstream health window is ~50s; first-time Postgres init on small VMs often needs longer. */
function patchDbHealthcheckForSlowInit(compose: string): string {
  // Unique to `db`: pg_isready on localhost, then env POSTGRES_HOST socket path (not used by other services).
  const re = /("localhost"\r?\n\s+\]\r?\n)\s+interval: 5s\r?\n\s+timeout: 5s\r?\n\s+retries: 10\r?\n(\s+environment:\r?\n\s+POSTGRES_HOST: \/var\/run\/postgresql)/;
  if (!re.test(compose)) return compose;
...
const SUPASCALE_COMPOSE_OVERRIDE = `# Generated by SupaScale — Postgres needs more than Docker default shm on many hosts.
services:
  db:
    shm_size: "512mb"
`;
...
export async function writeSupabaseStack(opts: { slug: string; kongHost: string; studioHost?: string | null }) {
  ...
  const composeBody = patchDbHealthcheckForSlowInit(compose.replace(/^name:\s*.*$/m, `name: supascale_${opts.slug}`));
  await fs.writeFile(path.join(dir, "docker-compose.yml"), composeBody, "utf8");
  await fs.writeFile(path.join(dir, "docker-compose.supascale.yml"), SUPASCALE_COMPOSE_OVERRIDE, "utf8");
